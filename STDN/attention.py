    ...
    def call(self, inputs, mask=None):
        ...
        if isinstance(inputs, list) and len(inputs) == 2:
            ...
            if self.method is None:
                ...
                hidden = tf.linalg.matmul(memory, self.Wh) + tf.expand_dims(tf.linalg.matmul(query, self.Wq), 1)  
                hidden = tf.tanh(hidden)
                s = tf.squeeze(tf.linalg.matmul(hidden, self.v), -1)
            elif self.method == 'ga':
                #s = K.sum(K.expand_dims(K.dot(query, self.Wq), 1) * memory, axis=-1)
                s = tf.reduce_sum(tf.expand_dims(tf.linalg.matmul(query, self.Wq), 1) * memory, axis=-1)  
            else:
                #s = K.squeeze(K.dot(memory, self.v), -1)
                s = tf.squeeze(tf.linalg.matmul(memory, self.v), -1) 
            if mask is not None:
                mask = mask[0]
        else:
            ...
            if self.method is None:
                ...
                #hidden = K.tanh(hidden)
                #s = K.squeeze(K.dot(hidden, self.v), -1)
                hidden = tf.linalg.matmul(memory, self.Wh)  
                hidden = tf.tanh(hidden)
                s = tf.squeeze(tf.linalg.matmul(hidden, self.v), -1)  
            elif self.method == 'ga':
